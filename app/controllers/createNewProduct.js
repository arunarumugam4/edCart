   ...
   console.log(req.body);
	...
	router.post('/deleteproduct',isLoggedIn,isProductAvailable,function(req, res){
        ...
        userModel.findById(req.user.id, function(err, user){
             ...
             // FIND THE PRODUCT IN THE USER PRODUCTS AND REMOVE IT FROM USER UPLOADED PRODUCTS
             let indexOfTheProduct = user.local.userProducts.indexOf(req.body.id);
            

             // THIS CHECKS WEATHER REQUESTED USER IS A VALID OWNER OR NOT

             if(indexOfTheProduct===-1){
                 // NOT AUTHORIZED TO DELETE THIS PRODUCT;
                 let response = responseGenerator(true,"your or not allowed to delete this product",400, null);
             ...
             user.save(function(err){
                ...
                // CHECK THE PRODUCT IN THE PRODUCT MODEL AND REMOVE IT
